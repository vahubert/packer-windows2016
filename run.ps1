...
if (test-path $isoFolder){
  remove-item $isoFolder -Force
}

New-IsoFile -Path .\answer.iso .\scripts\,.\answer_files\2016-hyperv2\Autounattend.xml

if (!$builder) {
packer build -var-file="variables.json" -on-error=ask windows2016.json
}
else {
packer build -var-file="variables.json" -only="$builder" -on-error=ask windows2016.json
}
